...
应用内置了 **150+ 条** Windows RAS 错误码的中文翻译，并采用固定间隔持续重连策略：

- **统一重试** — 所有拨号失败均按配置的重试间隔自动重连，不区分错误类型
- **达到上限** — 达最大重试次数后等待 30 秒，自动重置计数器继续重连（0 = 无限重试）
- **连接检测** — 持续轮询系统 RAS 状态，快速发现外部断开和休眠唤醒后的连接中断
